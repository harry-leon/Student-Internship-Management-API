...
public interface WeeklyReportRepository extends JpaRepository<WeeklyProgressReport, Integer> {
    ...
    long countByStatus(WeeklyReportStatus status);

    @Query("SELECT COUNT(r) FROM WeeklyProgressReport r WHERE r.assignment.phase.phaseId = :phaseId")
    long countByPhasePhaseId(@Param("phaseId") Integer phaseId);

    @Query("SELECT COUNT(r) FROM WeeklyProgressReport r WHERE r.assignment.phase.phaseId = :phaseId AND r.status = :status")
    long countByPhasePhaseIdAndStatus(@Param("phaseId") Integer phaseId, @Param("status") WeeklyReportStatus status);
}
