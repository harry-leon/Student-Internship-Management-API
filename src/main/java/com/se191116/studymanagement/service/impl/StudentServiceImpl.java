...
public class StudentServiceImpl implements StudentService {
    ...
    public Page<StudentResponse> getStudents(Pageable pageable) {
        return getStudents(null, null, pageable);
    }

    @Override
    public Page<StudentResponse> getStudents(String search, String major, Pageable pageable) {
        User currentUser = getCurrentUser(); // Lay user dang dang nhap
        Page<Student> students;
        String trimmedSearch = (search != null && !search.trim().isEmpty()) ? search.trim() : null;
        String trimmedMajor = (major != null && !major.trim().isEmpty() && !"ALL".equalsIgnoreCase(major.trim())) ? major.trim() : null;

        if (currentUser.getRole().equals(UserRole.MENTOR)) {
            if (trimmedSearch != null || trimmedMajor != null) {
                students = studentRepository.findMentorStudentsFiltered(currentUser.getUserId(), trimmedSearch, trimmedMajor, pageable);
            } else {
                students = studentRepository.findStudentByMentorId(currentUser.getUserId(), pageable);
            }
        } else {
            if (trimmedSearch != null || trimmedMajor != null) {
                students = studentRepository.findStudentsFiltered(trimmedSearch, trimmedMajor, pageable);
            } else {
