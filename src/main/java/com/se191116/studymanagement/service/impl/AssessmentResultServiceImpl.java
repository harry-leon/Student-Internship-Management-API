...
public class AssessmentResultServiceImpl implements AssessmentResultService {
    ...
    @Override
    public AssessmentResultResponse getAssessmentResultById(Integer resultId) {
        User currentUser = getCurrentUser();
        ...
        if (currentUser.getRole() == UserRole.STUDENT) {
            featureFlagService.requireFeatureEnabledForRole("STUDENT_VIEW_SCORE_ENABLED", currentUser.getRole());
            if (result.getAssignment() == null || result.getAssignment().getStudent() == null
                    || !java.util.Objects.equals(result.getAssignment().getStudent().getStudentId(), currentUser.getUserId())) {
                throw new AccessDeniedException("You can only view your own assessment results");
            }
        } else if (currentUser.getRole() == UserRole.MENTOR) {
            if (result.getAssignment() == null || result.getAssignment().getMentor() == null
                    || !java.util.Objects.equals(result.getAssignment().getMentor().getMentorId(), currentUser.getUserId())) {
                throw new AccessDeniedException("You can only view assessment results for your assigned students");
            }
        }
    ...
    @Override
    public List<AssessmentResultResponse> getAssessmentResultsByAssignmentAndRound(Integer assignmentId, Integer roundId) {
        User currentUser = getCurrentUser();
        List<AssessmentResult> results;

        if (currentUser.getRole() == UserRole.ADMIN) {
            results = assessmentResultRepository.findByAssignmentAndRound(assignmentId, roundId);
        } else if (currentUser.getRole() == UserRole.MENTOR) {
            results = assessmentResultRepository.findByAssignmentAndRound(assignmentId, roundId);
        } else {
            featureFlagService.requireFeatureEnabledForRole("STUDENT_VIEW_SCORE_ENABLED", currentUser.getRole());
    ...
    @Override
    public List<AssessmentResultResponse> getAssessmentResultsByPhase(Integer phaseId) {
        User currentUser = getCurrentUser();
        List<AssessmentResult> results;

        if (currentUser.getRole() == UserRole.ADMIN) {
            results = assessmentResultRepository.findByPhase(phaseId);
        } else if (currentUser.getRole() == UserRole.MENTOR) {
            results = assessmentResultRepository.findByAssignmentMentorMentorId(currentUser.getUserId(), Pageable.unpaged())
                    .stream()
                    .filter(ar -> ar.getAssignment().getPhase().getPhaseId().equals(phaseId))
                    .toList();
        } else {
            featureFlagService.requireFeatureEnabledForRole("STUDENT_VIEW_SCORE_ENABLED", currentUser.getRole());
            results = assessmentResultRepository.findByAssignmentStudentStudentId(currentUser.getUserId(), Pageable.unpaged())
                    .stream()
                    .filter(ar -> ar.getAssignment().getPhase().getPhaseId().equals(phaseId))
                    .toList();
        }
