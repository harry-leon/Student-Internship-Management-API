...
public class GlobalExceptionHandler {
    ...
    public ResponseEntity<ErrorResponse> handleInvalidStateTransitionException(InvalidStateTransitionException e, HttpServletRequest request) {
        log.warn("Invalid state transition: method={}, path={}, reason={}", request.getMethod(), request.getRequestURI(), e.getMessage());
        return buildResponse(HttpStatus.UNPROCESSABLE_ENTITY, ErrorCode.INVALID_STATE_TRANSITION, e.getMessage(), null);
    }
    ...
    public ResponseEntity<ErrorResponse> handleOptimisticLockException(Exception e, HttpServletRequest request) {
        log.warn("Concurrent update conflict: method={}, path={}, reason={}", request.getMethod(), request.getRequestURI(), e.getMessage());
        return buildResponse(HttpStatus.CONFLICT, ErrorCode.CONCURRENT_UPDATE, "Dữ liệu đã bị thay đổi bởi thao tác khác. Vui lòng tải lại trang và thử lại.", null);
    }
